...
class VAE(nn.Module):
    ...
    def sample_with_reparametrization(self, mu: TensorType['batch_size', 'latent_dim'], 
        ...
        """
        ##########################################################
        eps = torch.normal(mean=torch.zeros(mu.shape, device=mu.device), std=torch.ones(mu.shape, device=mu.device))
        z_prime = torch.exp(logsigma) * eps + mu
        return z_prime
        ##########################################################
    ...
    def kl_divergence(self, mu: TensorType['batch_size', 'latent_dim'], logsigma: TensorType['batch_size', 'latent_dim']) -> TensorType['batch_size']:
        ...
        kl = (sigma_squared.sum(axis=1) + torch.sum(mu ** 2, axis=1) - torch.log(sigma_squared).sum(axis=1) - self.latent_dim) / 2
    ...
    def elbo(self, x: TensorType['batch_size', 'input_dim']) -> TensorType['batch_size']:
        ...
        """
        ##########################################################
        mu, logsigma = self.encoder(x) # phi
        z_prime = self.sample_with_reparametrization(mu, logsigma)
        theta = self.decoder(z_prime)
        # not to calculate the mean over batch in BCE
        recon_loss = nn.functional.binary_cross_entropy(theta, x, reduction='none')
        recon_loss_per_sample = recon_loss.sum(dim=1)  # sum over input_dim
        elbo_mc = -recon_loss_per_sample - self.kl_divergence(mu, logsigma)
    ...
    def sample(self, num_samples: int, device: str='cpu') -> Tuple[
        ...
        """
        ##########################################################
        # z = torch.normal(mean=torch.zeros(num_samples, device=device), std=torch.ones(num_samples, device=device))
